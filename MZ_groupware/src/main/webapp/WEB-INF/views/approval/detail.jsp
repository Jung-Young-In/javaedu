			...
			<div class="card card-custom" id="kt_page_sticky_card">
				<div class="card-header">
					...
					<div class="card-toolbar" id="buttonArea">
						<div class="btn-group">
							<button type="button" class="btn btn-outline-primary font-weight-bold mr-2" style="width: 70px;" id="confirmButton" onclick="sweAlert('', 'app');">결재</button>
						</div>
						<div class="btn-group">
							<button type="button" class="btn btn-outline-warning font-weight-bold mr-2" style="width: 70px;" id="rejectButton" onclick="sweAlert('', 'rej');">반려</button>
						</div>
					</div>
				</div>
...
<%-- 							<form enctype="multipart/form-data" role="form" method="post" action="<%=request.getContextPath()%>/approval/regist.do" name="registForm"> --%>
									...
									<table id="lineTh" class="table table-bordered mb-3" style="border:0">
										...
										<tbody>
											...
											<tr style="height: 10px;">
												...
												<td id="firstTd">
													<input readonly="readonly" type="text" name="firstEmp" class="form-control px-0" value="${first.empName}" style="border: none; height: 9.75px; text-align: center;">
													<input type="hidden" name="firstEmpNo" value="${first.empNo }">
													<input type="hidden" name="appLevel" class="form-control" value="1">
												</td>
												<td id="secondTd">
													<input readonly="readonly" type="text" name="secondEmp" class="form-control px-0" value="${second.empName}" style="border: none; height: 9.75px; text-align: center;">
													<input type="hidden" name="secondEmpNo" value="${second.empNo }">
													<input type="hidden" name="appLevel" class="form-control" value="2">
												</td>
												<td id="thirdTd">
													<input readonly="readonly" type="text" name="thirdEmp" class="form-control px-0" value="${third.empName}" style="border: none; height: 9.75px; text-align: center;">
											...
											<tr>
												<td id="helpTd" class="text-center align-middle" style="height: 37px;">
													<c:forEach items="${help}" var="help">
														<input readonly="readonly" type="text" name="helpEmp" class="form-control" value="${help.empName}" style="border: none; height: 9.75px; text-align: center;">
								...
								<table id="subTh" class="table table-bordered mb-5">
										...
										<tr>
											<th >제목</th>
											<td colspan="3">
												<input readonly="readonly" type="text" id="appTitle" name='appTitle' class="form-control" value="${approval.appTitle }" style="border: none; height: 9.75px;">
											</td>
										</tr>
										<tr>
											<th>결재상태</th>
											<td>
												<input readonly="readonly" type="text" value="${approval.appStatus }" id="appStatus" name='appStatus' class="form-control"  style="border: none; height: 9.75px;">																	
											</td>
											<th>결재양식코드</th>
											<td>
												<input readonly="readonly" type="text" value="${approval.docId }" id="docId" name='docId' class="form-control" style="border: none; height: 9.75px;">																	
											</td>
										</tr>
										<tr>
											<th>상신자</th>
											<td>
												<input readonly="readonly" type="text" value="${approval.empName }" name="empName" class="form-control" style="border: none; height: 9.75px;">
<%-- 												<input type="hidden" value="${loginUser.empNo }" id="empNo" name='empNo' class="form-control"> --%>
											</td>
											<th>소속부서</th>
											<td>
												<input readonly="readonly" type="text" value="${approval.deptName }" class="form-control" style="border: none; height: 9.75px;">
											</td>
										</tr>
								</table>
							</div>
		               ...
		               <div class="card card-custom" style="border: 1px solid #E4E6EF;">
	                    ...
	                    <div class="card card-custom" style="border: 1px solid #E4E6EF;">
							...
							<div>
								...
								<input class="form-control col-10 float-left" type="text"	placeholder="댓글을 입력하세요." id="newApprovalReplytext">
...
<script>
if (($("input[name='firstEmpNo']").val() == "${loginUser.empNo}") && ($("#firstAppSign").val() == 'Y')
	|| ($("input[name='secondEmpNo']").val() == "${loginUser.empNo}") && ($("#secondAppSign").val() == 'Y')
	|| ($("input[name='thirdEmpNo']").val() == "${loginUser.empNo}") && ($("#thirdAppSign").val() == 'Y')
	|| ($("input[name='firstEmpNo']").val() == "${loginUser.empNo}") && ($("#firstAppSign").val() == 'X')) {

	$("#confirmButton").css("display", "none");
	$("#rejectButton").css("display", "none");
	
	var str = '<button class="btn btn-secondary" disabled>이미 결재한 문서입니다</button>';
	
	$('#buttonArea').append(str);
}

if (($("input[name='firstEmp']").val() == "${loginUser.empName}") && ($("#firstAppSign").val() == 'R')
	|| ($("input[name='secondEmp']").val() == "${loginUser.empName}") && ($("#secondAppSign").val() == 'R')
	|| ($("input[name='thirdEmp']").val() == "${loginUser.empName}") && ($("#thirdAppSign").val() == 'R')) {

	$("#confirmButton").css("display", "none");
	$("#rejectButton").css("display", "none");
	
	var str = '<button class="btn btn-secondary" disabled>이미 반려한 문서입니다</button>';
	
	$('#buttonArea').append(str);
}

if ($("input[name='empName']").val() == "${loginUser.empName}") {
	$('#confirmButton').css("display", "none");
// 	$('#confirmButton').css("color", "gray").css("border", "1px solid gray");
	$('#rejectButton').css("display", "none");
// 	$('#rejectButton').css("color", "gray").css("border", "1px solid gray");
}

var idx = 0;

function sweAlert(str, typ) {
	if(str == "") {
		str = '<input type="password" id="swal-input1" class="swal2-input m-0" style="height: 30px; margin-bottom: 10px;">';
	}
	
	if (idx > 2 ) {
		Swal.fire({html : "비밀번호 3회 잘못 입력하였습니다.<br>비밀번호가 기억나지 않을 시 <br>관리자에게 문의하세요.", confirmButtonText: "확인"});
		idx = 0;
		return;
	}
	...
	Swal.fire({
	    ...
	    if (result.value) {
	    	if (document.getElementById('swal-input1').value == ${loginUser.appPwd}) {
	    		if(typ == "app") {
		    		app_go();
	    		}
	    		if(typ == "rej") {
	    		...
	    		idx++;
				var html = '<p style="color: red; font-size: 12px;">비밀번호를 '+ idx +'회 잘못 입력하였습니다.<p>'
			    	+ '<input type="password" id="swal-input1" class="swal2-input m-0" style="height: 30px; margin-bottom: 10px;">';
		    	if(typ == "app") {
				    sweAlert(html, "app");
	    		}
	    		if(typ == "rej") {
...
function app_go() {
	...
	if($("[name='thirdEmpNo']").val() == $("[name='empNo']").val()) {
		$("#flag").val("Y");
	}else if("${second.empNo}" == "${loginUser.empNo}" && "${first.appSign}" == "N") {
		$("#flag").val("X");
	}else{
		$("#flag").val("N");
	}
	...
	$.ajax({
		...
		success : function(data) {
			...
			var str = '<button class="btn btn-secondary" disabled><i class="fas fa-check-circle"></i>결재 완료</button>'
			var alert = '<script>Swal.fire({'
			  		  ...
			  		  + 'timer: 1500});'
			  		  + '</' + 'script>'
			var status = '<span style="color:red;" class="label label-xl label-outline-danger font-weight-bolder label-pill label-inline">결재</span>'
			
			if($("#flag").val() == 'X') {
				var x = '<span style="color:#FFA800;" class="label label-xl label-outline-warning font-weight-bolder label-pill label-inline">전결</span>';
				$('#${first.empNo} > span').remove();
				$('#${first.empNo}').append(x);
				socket.send("전자결재,${first.empNo},${approval.appTitle },전결된 결재문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
				socket.send("전자결재,${third.empNo},${approval.appTitle },수신된 결재문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
			}
			...
			if($("#flag").val() == "Y" && '${loginUser.empNo}' == "${third.empNo}") {
				socket.send("전자결재,${approval.empNo },${approval.appTitle },결재완료된 문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
				<c:forEach items="${read}" var="read">
					socket.send("전자결재,${read.empNo},${approval.appTitle },결재완료된 문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
				</c:forEach>
			}
			if($("#flag").val() == "N" && '${loginUser.empNo}' == "${second.empNo}") {
				socket.send("전자결재,${third.empNo},${approval.appTitle },수신된 결재문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
			}
			if($("#flag").val() == "N" && '${loginUser.empNo}' == "${first.empNo}") {
				socket.send("전자결재,${second.empNo},${approval.appTitle },수신된 결재문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
			}
		},
		error : function(err) {
			console.log('에러 발생');
...
$.ajax({
	...
	success : function(data) {
			console.log(data);
			$('#rejectButton').remove();
			var str = '<button class="btn btn-secondary" disabled><i class="fas fa-check-circle"></i>반려 완료</button>'
			var alert = '<script>Swal.fire({'
			  		  + 'icon: "success",'
			  		  + 'title: "확인 처리되었습니다.",'
			  		  + 'showConfirmButton: false,'
			...
			$('#buttonArea').append(alert);	
			if($("#flag").val() == "R") {
				socket.send("전자결재,${approval.empName },${approval.appTitle },반려된 결재문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
			}
		},
		error : function(err) {
			console.log('에러 발생');
