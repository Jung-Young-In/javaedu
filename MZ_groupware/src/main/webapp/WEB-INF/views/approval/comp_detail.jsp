...
function app_go() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if($("#flag").val() == 'X') {
				var x = '<span style="color:red;" class="label label-xl label-outline-danger font-weight-bolder label-pill label-inline">전결</span>';
			...
			if($("#flag").val() == "Y") {
				socket.send("전자결재,${approval.empNo }/${first.empNo }/${second.empNo },${approval.appTitle },결재완료된 문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
			<c:forEach items="${help}" var="help">
				socket.send("전자결재,${help.empNo},${approval.appTitle },결재완료된 문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
			</c:forEach>
			<c:forEach items="${read}" var="read">
...
$.ajax({
	...
	success : function(data) {
			...
			$('#buttonArea').append(alert);	
			if($("#flag").val() == "R") {
				socket.send("전자결재,${approval.empName }/${first.empNo }/${second.empNo }/${third.empNo },${approval.appTitle },반려된 결재문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
			<c:forEach items="${help}" var="help">
				socket.send("전자결재,${help.empNo},${approval.appTitle },반려된 결재문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
			</c:forEach>
			<c:forEach items="${read}" var="read">
				socket.send("전자결재,${read.empNo},${approval.appTitle },반려된 결재문서가 있습니다.,<%=request.getContextPath()%>/approval/lib/ing/detail.do?appNo=${approval.appNo}");
			</c:forEach>
			}
		},
		error : function(err) {
