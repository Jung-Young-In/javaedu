...
<div class="d-flex flex-column-fluid">
		...
		<div class="container-fluid">
			...
			<div class="card card-custom" id="kt_page_sticky_card">
				<div class="card-header">
					...
					<div class="card-toolbar">
						<button type="button" class="btn btn-light btn-shadow-hover font-weight-bold mr-2" onclick="fn_auto();" style="border: none; margin-right: 10px;">auto</button>
			...
			<!-- 						일반 결재 문서 -->
							...
							<input type="hidden" name="empNo" class="form-control" value="${loginUser.empNo }">
									...
									<table id="lineTh" class="table table-bordered mb-3" style="border: 0px;">
										...
										<tbody>
											...
											<tr style="height: 10px;">
												<td>
													<input type="text" class="form-control" value="${loginUser.empName}" style="border: none; height: 9.75px; text-align: center;">
												</td>
												<td id="firstTd">
								...
								<table id="subTh" class="table table-bordered mb-5">
										...
										<tr>
											<th>제목</th>
											<td colspan="3">
												<input type="text" id="appTitle" name='appTitle' class="form-control" placeholder="제목을 입력하세요" style="border: none; height: 9.75px;">
											</td>
										</tr>
										<tr>
											<th>결재상태</th>
											<td>
												<input type="text" value="진행중" id="appStatus" name='appStatus' class="form-control" style="border: none; height: 9.75px;">																	
											</td>
											<th>결재양식코드</th>
											<td>
												<input type="text" value="${doc.docId }" id="docId" name='docId' class="form-control" style="border: none; height: 9.75px;" readonly="readonly">																	
											</td>
										</tr>
										<tr>
											<th>상신자</th>
		               ...
		               <input type="hidden" name="vacaStartDay" />
		               <input type="hidden" name="vacaEndDay" />
		               <input type="hidden" name="vacaDay" />
		               <input type="hidden" name="vacaReason" />
					</form>
				</div>
			</div>
		</div>
...
<script>
function fn_auto() {
	$('input[name="appTitle"]').val("[근태결재문서]근태조정 상신의 건");
}
...
function regist_go() {
	...
	if("${doc.docId }" == 'C01') {
		if($(".note-editor #vacaStartDay").text() == '') {
			alert("휴가 시작일을 입력하세요.");
			return;
		}
		if($(".note-editor #vacaEndDay").text() == '') {
			alert("휴가 종료일을 입력하세요.");
			return;
		}
		if($(".note-editor #vacaDay").text() == '') {
			alert("연차 일수를 입력하세요.");
			return;
		}
		if($(".note-editor #vacaReason").text() == '') {
			alert("휴가 사유를 입력하세요.");
			return;
		}
		$("[name='vacaStartDay']").val($(".note-editor #vacaStartDay").text().trim());
		$("[name='vacaEndDay']").val($(".note-editor #vacaEndDay").text().trim());
		$("[name='vacaDay']").val($(".note-editor #vacaDay").text().trim());
		$("[name='vacaReason']").val($(".note-editor #vacaReason").text().trim());
	}
...
function appList_go(type) {
	...
	if(type == 'first') {
		...
		refStr += '<tr style="left: 0px;" id="listTr" onclick="remove_go(this);">'
 				+ 	'<td scope="col">'
		...
		helpStr += '<tr style="left: 0px;" id="listTr" onclick="remove_go(this);">'
				+ 	'<td scope="col">'
		...
		readStr += '<tr style="left: 0px;" id="listTr" onclick="remove_go(this);">'
				+ 	'<td scope="col">'
